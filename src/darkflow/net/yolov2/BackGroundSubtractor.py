...
def test2():
    ...
    videoWriter = cv2.VideoWriter(
        'videobg.avi', fourcc, fps, (width, height))
    while (cap.isOpened()):
        ret, frame = cap.read()
        ...
        print(type(fgmask))
        if type(fgmask) is not np.ndarray:
            fgmask = cv2.imread(fgmask)
        #cv2.imshow('frame', fgmask)
        fgmask = cv2.merge((fgmask,fgmask,fgmask))
        frame[np.where((fgmask==[0,0,0]).all(axis=2))] = [0,0,0]
        frame[np.where((fgmask == [127, 127, 127]).all(axis=2))] = [0, 0, 0]
