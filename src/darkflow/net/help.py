...
def camera(self):
    ...
    # Loop through frames -- parte importante
    while camera.isOpened():
        ...
        if frame is None:
            ...
            break
        #frameTwo = frame.copy()
        #fgmask = fgbg.apply(frameTwo)
        #fgmask = cv2.merge((fgmask, fgmask, fgmask))
        #frameTwo[np.where((fgmask == [0, 0, 0]).all(axis=2))] = [0, 0, 0]
        #frameTwo[np.where((fgmask == [127, 127, 127]).all(axis=2))] = [0, 0, 0]
        if elapsed >= 0 and elapsed <= 150 :
            if frame is None:
                print ('\nEnd of Video')
                break
            #upper,lower = featureext.decompose(frame)
            preprocessed = self.framework.preprocess(frame)
            buffer_inp.append(frame)
            buffer_pre.append(preprocessed)
            commonList.tempo = (elapsed/30)
            # Only process and imshow when queue is full
            if elapsed % self.FLAGS.queue == 0:
                feed_dict = {self.inp: buffer_pre}
                net_out = self.sess.run(self.out, feed_dict)
                for img, single_out in zip(buffer_inp, net_out):
                    postprocessed = self.framework.postprocess(0,single_out, img, False)
                    if SaveVideo:
            ...
            if elapsed % 5 == 0:
                ...
                sys.stdout.write('Size: {} '.format(len(commonList.recognized)))
                sys.stdout.flush()
            if file == 0: #camera window
    ...
    if SaveVideo:
        videoWriter.release()
    f = open("times.txt","w+")
    for element in commonList.recognized:
    	f.write(str(element[2])+str(element[1])+" appears at time: " + str(element[3]) + " and disappears at time: " + str(element[4])+str("\n"))
    f.close()
    camera.release()
    if file == 0: #camera window
